...
**Phase 2** — pgvector RAG (40 PSLE questions, grade-filtered context retrieval), Redis `@Cacheable` (24h TTL), `OllamaConfig` interceptor (Spring AI 2.0.0-M2 `think` bug workaround).
...
**Phase 8** — `ExplanationMode` enum (ORIGINAL / SOCRATIC), planner-derived Socratic questioning, client-side OCR with `tesseract.js`, CJK font/IME hardening, and deterministic arithmetic fast path for trivial questions.

---

**Phase 10** — Dynamic achievement wall (practice streaks, mastery milestones, reflection badges), adaptive learning-path API, star-map visualization, and one-click challenge handoff from recommendations to the solve screen.

---

**Phase 9 (current skeleton)** — Mistake ledger endpoint (`GET /api/v1/records/mistakes`) and export payload endpoint (`GET /api/v1/records/{recordId}/export`) are implemented, with dedicated frontend `Mistakes` page for filter + preview workflow.
...
- Error notebook (`GET /api/v1/records/mistakes`, rating ≤ 2) — skeleton delivered
