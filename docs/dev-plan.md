...
| # | 任务 | 验证方式 |
|:--|:-----|:---------|
| 8.3 | `MathSolverOrchestrator` 增加 `TutoringAgent`，生成分步引导问题而非直给答案 | 开启启发模式，返回结果包含引导问题链 |
| 8.4 | 前端实现“分步解锁”交互，支持学生提交引导问题的答案并获得即时反馈 | 学生互动完成解题，非一次性展示结果 |

### Task 8.5: 多解法路径对比
| # | 任务 | 验证方式 |
|:--|:-----|:---------|
| 8.5 | 优化 Prompt，支持生成 Model Method 与 Algebra 两种对比解法 | 结果区显示“多种思路”切换 Tab |
...
| 优先级 | 任务 | 状态 |
|:-------|:-----|:-----|
| **P0 (Critical)** | **Phase 7: 本地性能优化** (语义缓存、重试机制) | 待启动 |
| **P1 (High)** | **Phase 8.1-8.2: OCR 识题** (多模态升级) | 待启动 |
| **P1 (High)** | **Phase 8.3-8.4: 启发式教学** (核心差异化) | 待启动 |
