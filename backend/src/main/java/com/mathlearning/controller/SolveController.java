...
/**
 * Solve endpoints providing both structured multi-agent pipeline and SSE
 * streaming. The structured endpoint uses Redis caching (24h TTL).
 */
@RestController
...
public class SolveController {
	...
	/**
	 * Streams the multi-agent pipeline result as Server-Sent Events. Each section
	 * (parentGuide, childScript, barModel, knowledgeTags) is sent as a separate SSE
	 * event with JSON payload {"type": "...", "content": "..."}. Results benefit
			...
			try {
				...
				if (result.parentGuide() != null && !result.parentGuide().isBlank()) {
					events.add(objectMapper
							.writeValueAsString(Map.of("type", "parent_guide", "content", result.parentGuide())));
				}
				if (result.childScript() != null && !result.childScript().isBlank()) {
					events.add(objectMapper
							.writeValueAsString(Map.of("type", "child_script", "content", result.childScript())));
				}
				if (result.barModelJson() != null && !result.barModelJson().isBlank()) {
					events.add(objectMapper
							.writeValueAsString(Map.of("type", "bar_model", "content", result.barModelJson())));
				}
				if (result.knowledgeTags() != null && !result.knowledgeTags().isEmpty()) {
					events.add(objectMapper.writeValueAsString(
							Map.of("type", "knowledge_tags", "content", String.join(", ", result.knowledgeTags()))));
				}
				events.add("[DONE]");
				...
				try {
					return Flux.just(objectMapper.writeValueAsString(Map.of("type", "error", "content",
							e.getMessage() != null ? e.getMessage() : "Unknown error")), "[DONE]");
				} catch (Exception ex) {
