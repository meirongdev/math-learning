 ...
 *            usage)
 * @param mode
 *            explanation style — ORIGINAL (direct) or SOCRATIC (heuristic
 *            questions); defaults to ORIGINAL when null
 */
public record SolveRequest(
		...
		UUID studentId, ExplanationMode mode) {

	public SolveRequest(String question, int grade) {
		this(question, grade, null, null);
	}

	/**
	 * Returns the effective explanation mode, defaulting to ORIGINAL when not
	 * specified.
	 */
	public ExplanationMode effectiveMode() {
		return mode != null ? mode : ExplanationMode.ORIGINAL;
	}
}
