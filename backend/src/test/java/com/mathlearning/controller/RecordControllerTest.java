...
class RecordControllerTest extends AbstractIntegrationTest {
	...
	void setUp() {
		...
		User outsider = userRepository
				.save(User.builder().email("outsider-" + suffix + "@example.com").password("encoded").build());
		...
		ownerLowRatedRecord = solveRecordRepository.save(SolveRecord.builder().student(ownerStudent)
				.questionText("What is 1/2 + 1/3?").parentGuide("Use LCD").childScript("Find common denominator")
				.knowledgeTags(List.of("frac.add_sub")).rating(2).createdAt(OffsetDateTime.now().minusDays(1)).build());

		solveRecordRepository.save(SolveRecord.builder().student(ownerStudent).questionText("What is 3 + 4?")
				.knowledgeTags(List.of("whole_numbers")).rating(5).createdAt(OffsetDateTime.now()).build());

		solveRecordRepository.save(SolveRecord.builder().student(outsiderStudent).questionText("What is 5/8 + 1/8?")
				.knowledgeTags(List.of("frac.add_sub")).rating(1).createdAt(OffsetDateTime.now().minusHours(3))
				.build());
	}
	...
	void getMistakes_ReturnsOnlyOwnerLowRatedRecords() throws Exception {
		mockMvc.perform(
				get("/api/v1/records/mistakes").param("studentId", ownerLowRatedRecord.getStudent().getId().toString())
				...
				.andExpect(jsonPath("$.records[*].id")
						.value(org.hamcrest.Matchers.hasItem(ownerLowRatedRecord.getId().toString())));
	}

	@Test
	void exportRecord_ReturnsPreviewPayloadForOwner() throws Exception {
		mockMvc.perform(get("/api/v1/records/{recordId}/export", ownerLowRatedRecord.getId()).header("Authorization",
				"Bearer " + ownerToken)).andExpect(status().isOk())
	...
	void exportRecord_OutsiderGets404() throws Exception {
		mockMvc.perform(get("/api/v1/records/{recordId}/export", ownerLowRatedRecord.getId()).header("Authorization",
				"Bearer " + outsiderToken)).andExpect(status().isNotFound());
	}
	...
	void rateRecord_OutsiderGets404() throws Exception {
		mockMvc.perform(patch("/api/v1/records/{recordId}/rating", ownerLowRatedRecord.getId())
				.header("Authorization", "Bearer " + outsiderToken).contentType(MediaType.APPLICATION_JSON).content("""
						{
						  "rating": 4
						}
